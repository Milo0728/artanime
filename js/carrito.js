let añadirElementos = () => {
    ...
    let objetosArreglo = JSON.parse( localStorage.getItem('productosCarrito') );
...
function funcionBoton( index ) {
    ...
    let figuras = JSON.parse( localStorage.getItem("figuras"));
    console.log( index );
        console.log( figuras[ parseInt( index )] );
        validarSiExisteFigura( figuras[ parseInt(index )]);
        añadirElementos( figuras[ parseInt( index )] );  


    // index.preventDefault()
}

function validarSiExisteFigura ( figuras ){
    if( JSON.parse( localStorage.getItem("productosCarrito") ) == undefined ){
        localArr =  [ figuras ];
        localStorage.setItem("productosCarrito", JSON.stringify( localArr ));
   }
   else{
       localArr = JSON.parse( localStorage.getItem("productosCarrito") );
