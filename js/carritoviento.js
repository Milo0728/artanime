let añadirElementosViento = () => {

    const carritoPadre = document.getElementById("cart");

    let objetosArreglo = JSON.parse( localStorage.getItem('productosCarritoViento') );
...
function funcionBotonViento( index ) {
    ...
    let figuras = JSON.parse( localStorage.getItem("figurasViento"));
    console.log( index );
        console.log( figuras[ parseInt( index )] );
        validarSiExisteFiguraViento( figuras[ parseInt(index )]);
        añadirElementosViento( figuras[ parseInt( index )] );  


    // index.preventDefault()
}

function validarSiExisteFiguraViento ( figuras ){
    if( JSON.parse( localStorage.getItem("productosCarritoViento") ) == undefined ){
        localArr =  [ figuras ];
        localStorage.setItem("productosCarritoViento", JSON.stringify( localArr ));
   }
   else{
       localArr = JSON.parse( localStorage.getItem("productosCarritoViento") );
